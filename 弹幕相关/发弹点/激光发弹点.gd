...
func bullet_spawn_logic():
	...
	for bull in bullets:
		var bp_size = RS.bullet_pics[spawn_bullet_type]\
		[spawn_bullet_color].get_size()/2
		bp_size.x = 0
		bp_size.y *= bull.scale.y
		bp_size = bp_size.rotated(deg_to_rad(bull.rotation))
		bull.real_position += bp_size
